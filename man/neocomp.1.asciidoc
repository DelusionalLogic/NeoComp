...
*--sw-opti*::
	Limit neocomp to repaint at most once every 1 / 'refresh_rate' second to boost performance. This should not be used with *--vsync* drm/opengl/opengl-oml as they essentially does *--sw-opti*'s job already, unless you wish to specify a lower refresh rate than the actual value.
...
It's possible to control neocomp via D-Bus messages, by running neocomp with *--dbus* and send messages to `com.github.chjj.neocomp.<DISPLAY>`. `<DISPLAY>` is the display used by neocomp, with all non-alphanumeric characters transformed to underscores. For `DISPLAY=:0.0` you should use `com.github.chjj.neocomp._0_0`, for example.
...
+
------------
$ neocomp -c --shadow-red 1 --shadow-green 1 --shadow-blue 1
------------
