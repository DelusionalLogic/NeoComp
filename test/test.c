...
void readAllEventsInto(struct X11Context* xctx, Vector* events) {
    struct Event ev;
    while(XEventsQueuedH(xctx->display, 0) > 0 || vector_size(&xctx->eventBuf) > xctx->readCursor) {
...
struct TestResult xorg__emit_nothing__bypassed_frame_requests_bypass() {
    struct X11Context ctx;
    struct Atoms atoms;
    Display* dpy = (void*)0x01;
    xorgContext_init(&ctx, dpy, 0, &atoms);
...
struct TestResult xorg__emit_nothing__unmapped_frame_requests_bypass() {
    struct X11Context ctx;
    struct Atoms atoms;
    Display* dpy = (void*)0x01;
    xorgContext_init(&ctx, dpy, 0, &atoms);
...
struct TestResult xorg__emit_nothing__unmapped_window_is_damaged() {
    struct X11Context ctx;
    struct Atoms atoms;
    Display* dpy = (void*)0x01;
    xorgContext_init(&ctx, dpy, 0, &atoms);
...
struct TestResult xorg__emit_damage__mapped_window_is_damaged() {
    struct X11Context ctx;
    struct Atoms atoms;
    Display* dpy = (void*)0x01;
    xorgContext_init(&ctx, dpy, 0, &atoms);
...
int main(int argc, char** argv) {
    ...
    TEST(xorg__emit_nothing__bypassed_frame_requests_bypass);
